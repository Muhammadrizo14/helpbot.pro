<template>
  <div v-if="questionsList.length === 0"
       class="database__questions-empty flex align-items-center justify-content-center flex-column">
    ...
    <Button icon="pi pi-plus" label="Добавить" class="px-4 py-3" @click="addQuestionModal=true"/>
  </div>

  <div class="pt-4" v-if="questionsList.length > 0">
    <div class="flex align-items-center justify-content-between">
      <InputText placeholder="Поиск..." class="border-none w-20rem"/>
      <Button label="Добавить" icon="pi pi-plus" @click="addQuestionModal=true"/>
    </div>
    <DataTable :value="questionsList" :tableClass="'users-data'" class="users-table pt-4">
      <template #footer>
        <p class="text-center">На любые другие вопросы бот будет отвечать по базе знаний</p>
      </template>
      <Column field="question" header="Главный вопрос" style="min-width: 480px"></Column>
      ...
      <Column field="answer" header="Ответ" style="width: 392.67px">
        <template #body="slotProps">
          <Button v-if="slotProps.data.answer === 'Перенаправить на оператора'"
      ...
      <Column style="width: 10%; min-width: 8rem" bodyStyle="text-align:center">
        <template #body="slotProps">
          <div class="flex gap-3">
            ...
            />
            <Button icon="pi pi-file-excel" outlined severity="danger" @click="()=> remove(slotProps.index)"/>
          </div>
        </template>
      </Column>
    </DataTable>
  ...
  <Dialog v-model:visible="addQuestionModal" modal :closable="false" :draggable="false">
    <template #header>
      <div class="pb-3">
    ...
    <img src="@/assets/images/icons/close.png" alt="Close" class="close-icon" @click="addQuestionModal = false">

    <form @submit.prevent="add">
      <div class="flex flex-column gap-2 pb-4">
        <label for="question" class="database-add__label">Главный вопрос</label>
        <InputText id="question" placeholder="Я забыл свой пароль..." :invalid="v$.question.$errors.length > 0"
                   v-model="data.question"/>
        <label for="login__form-title" v-for="error in v$.question.$errors" :key="error.$uid"
      ...
      <div class="flex flex-column gap-2 pb-4">
        ...
        <Textarea v-model="data.similar" class="max-w-30rem" rows="2" cols="30"
                  :invalid="v$.similar.$errors.length > 0"/>
        <label for="login__form-title" v-for="error in v$.similar.$errors" :key="error.$uid"
               style="color: var(--red)">{{ error.$message }}</label>
      </div>
      ...
      <div v-if="selectedAction.title === 'Ответ'" class="flex flex-column gap-2 pb-4">
        ...
        <InputText id="question" placeholder="Здесь должен быть ответ" v-model="data.answer"
...
const remove = (id) => {
  questionsList.value = questionsList.value.filter((question, index) => index !== id)
}
const add = async () => {
...
watch(selectedAction, (newAction) => {
  if (newAction.title === 'Перенаправить на оператора') {
    ...
    rules.answer.required = helpers.withMessage(customMessages.required, required);
  }
});
</script>
